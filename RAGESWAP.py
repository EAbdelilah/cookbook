...
class GeminiClient:
    ...
    def call_with_retry(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        Retries indefinitely for quota and transient errors to ensure task completion."""
        attempt = 0
        while True:
            try:
                ...
                if is_quota_error or is_transient_error:
                    if is_quota_error and attempt >= 5:
                        ...
                        logger.warning(f"  [Critical Quota Hit] Waiting for {wait_time:.2f}s (Attempt {attempt})...")
                    else:
                        # Exponential backoff capped at 60s to maintain persistence without runaway delays
                        wait_time = min(2 ** (attempt % 10), 60) + random.uniform(0, 2)
                        logger.info(f"  Transient error or Rate limit hit. Waiting for {wait_time:.2f}s (Attempt {attempt})...")
                    time.sleep(wait_time)
                else:
...
class RAGEngine:
    ...
    def process_question(self, question: str, chunk_embeddings: List[List[float]], chunks: List[str], grant_context: str, persona: str) -> str:
        ...
        try:
            ...
            return f"[Error] This question failed due to a non-retryable error: {e}"
