...
# On Linux/macOS: export GEMINI_API_KEY="your_key"
...
def make_api_call(payload, system_prompt_text, retries=3):
    ...
    """
    # NOTE: Changed to gemini-1.5-flash as gemini-2.5-flash is not a standard model name.
    ...
    for attempt in range(retries):
        try:
            # Add a small delay before every request to avoid burst limits
            # Free tier allows roughly 15 requests per minute, so 4s is safe.
            time.sleep(4)
            ...
            # If we hit the rate limit (429), wait longer and retry
            if "429" in str(e) or "quota" in str(e).lower():
                # Wait 60 seconds (the typical reset window) plus a little jitter
...
def searcher_ai(user_query, grant_context, all_questions):
    ...
    # Combine user query, grant context, and all questions for a more effective search
    ...
    try:
        ...
        # Use Gemini to summarize the search results (calls make_api_call to handle limits)
...
def select_best_documents(user_query, document_descriptions, searcher_results):
    ...
    # Validate that the selected IDs are actual document keys
